...
(() => {
    const app = {
        btns: ["argentine tango", "b-boy", "ballet", "belly", "bhangra", "carlton", "charleston", "disco", "dougie", "hip-hop", "hula", "gangnam style", "lindy hop", "line", "moonwalk", "paso doble", "salsa", "shimmy", "waltz"],
        colors: ["red", "orange", "yellow", "olive", "green", "blue", "violet", "purple", "pink", "brown"],
        load: () => {
            app.makeBtns(app.btns);
            $("#submit").on("click", event => {
                event.preventDefault();
                app.addBtn($("#searchTerm").val().toLowerCase());
                $("#searchTerm").val("");
            });
        ...
        getGifs: function() {
            $.ajax(`https://api.giphy.com/v1/gifs/search?api_key=qZKo14UVEsTDJkz1YpxVkj9cq7HxPC1N&limit=10&lang=en&q=${$(this).val()}+dance`).then(response => app.makeStills(response));
